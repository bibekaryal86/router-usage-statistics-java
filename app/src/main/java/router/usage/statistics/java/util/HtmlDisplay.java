...
public class HtmlDisplay {
  ...
  public static String getDisplay(
      List<Model> modelList, Model modelTotal, String selected, Set<String> yearMonthSet) {
  ...
  private static String getHead() {
    ...
    return headOpen + getMetaTitle() + getStyle() + headClose;
  }

  private static String getMetaTitle() {
    return "<meta charset=\"UTF-8\">" + "<title>Internet Usage Statistics</title>";
  }
  ...
  private static String currentMonthLink() {
    ...
    return divOpen + addLink("/", "BACK TO CURRENT MONTH") + DIV_CLOSE;
  }
  ...
  private static String getDataUsageTableHead(String selected) {
    ...
    String headerOne =
        "<tr><th colspan=\"4\" style=\"text-align: center;\">DATA OF SELECTED MONTH ("
            + selected
            + ") </th></tr>";
    return theadOpen + headerOne + getDataUsageTableRow(true, null) + theadClose;
  }
  ...
  private static String getDataUsageTableRow(boolean isHeader, Model model) {
    ...
    if (isHeader) {
      ...
      stringBuilder.append(
          getTableColumn(false, getFormattedDate(model.getDate(), model.getDay()), false));
  ...
  private static String getAvailableDataTable(Set<String> yearMonthSet) {
    String tableOpen = "<table>";
    String tableClose = "</table>";
    return tableOpen + getAvailableDataTableBody(yearMonthSet) + tableClose;
  }
  ...
  private static String getTableColumn(boolean isHeader, String value, boolean isAddLink) {
    ...
    if (isAddLink) {
      return thtdOpen + addLink("/?selected=" + value, value) + thtdClose;
    } else {
